...
constexpr float PINCH_CLOSE_ANGLE_DEG_L = 95.0f;
constexpr float PINCH_OPEN_ANGLE_DEG_R = 0.0f;
constexpr float PINCH_CLOSE_ANGLE_DEG_R = -95.0f;
  ...
  while (Serial.available()) {
    ...
    if (c == '\n') {
      ...
      if (rx_line.length() > 0) {
        ...
        // } else 
        if (handlePinchCommand(rx_line, pinch_cmd)) {
