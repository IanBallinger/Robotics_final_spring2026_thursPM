...
def setup_plot(map_: Map, planned_tasks: list[PlannedTask]):
    ...
    fig.tight_layout()
    return plt, fig, ax, artists, robot_line, robot_marker
...
def main():
    ...
    plt, fig, _, artists, robot_line, robot_marker = setup_plot(map_, planned_tasks)
    if not args.no_show:
        plt.ion()
    ...
    for i in range(1, args.max_ticks + 1):
        ...
        print(
            f"current_task = {current_task}, pos=({x:.3f}, {y:.3f}), psi={psi:.3f}, "
